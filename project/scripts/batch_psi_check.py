...
def run_psi_check():
    ...
    if current_data.empty or baseline_data.empty:
        print("Insufficient data for PSI check")
        return

    drift_report = {}
    
    # Check PSI for each numerical feature
    ...
    for feature in numerical_features:
        if feature in current_data.columns and feature in baseline_data.columns:
            psi = detector.calculate_psi(baseline_data[feature].values, current_data[feature].values)
            is_drift = rules.check_threshold('covariate', 'psi', psi)
            ...
            if is_drift:
                print(f"DRIFT DETECTED in {feature}: PSI = {psi}")
    
    # Save report
    with open(f'data/drift_reports/psi_report_{today}.json', 'w') as f:
